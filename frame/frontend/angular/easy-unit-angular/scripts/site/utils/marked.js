...
renderer.heading = function (text, level) {
  ...
  const isMarkedLabel = level === 3 && text.indexOf(`${projectConfig.component.prefix}-`) === 0;
  const isDirective = text[0] === '[' && text[text.length - 1] === ']';
  const isComponent = isMarkedLabel && !isDirective;
  const isService = text.indexOf(projectConfig.component.prefix.firstUpperCase()) === 0 && text.indexOf('Service') > -1;
